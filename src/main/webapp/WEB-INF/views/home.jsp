...
<script>
	$(function(){
		...
		$("#mainCategory").on("change",function() {
					...
					$.getJSON('/product/getCate/' + mainCateCode,
							function(data) {
								var template = Handlebars.compile($(
										"#subCateSearchTemplate").html());
...
<script id="subCateSearchTemplate" type="text/x-handlebars-template">
        ...
        -------------------------->
					...
					<div class="col-xs-3">
						<select id="mainCategory" name="mainCategory"
							...
							<c:forEach items="${prt_cateCode }" var="cate">
								<option value="${cate.category_code }"
								<c:out value="${cri.mainCategory  eq cate.category_code ? 'selected':''}" />>
	        	...
	        	<div class="box box-primary center">
					<div class="box-header"><strong>전체상품</strong></div>
	        		...
	        		<div class="text-center">
						<ul class="pagination">
							...
							<c:if test="${pageMaker.prev}">
								<li><a
									href="/search${pageMaker.makeCateSearch(pageMaker.startPage - 1) }">&laquo;</a></li>
							</c:if>
							...
							<c:forEach begin="${pageMaker.startPage }"
								...
								<li
									...
									<a href="/search${pageMaker.makeCateSearch(idx)}">${idx}</a>
								</li>
							</c:forEach>

							<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
								<li><a
									href="/search/${pageMaker.makeCateSearch(pageMaker.endPage +1) }">&raquo;</a></li>
							</c:if>
